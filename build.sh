...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.AmFmPmBasebandDemod.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.AmFmPmBasebandDemod-2.0.1
        tar czf ${tmpdir}/rh.AmFmPmBasebandDemod-2.0.1.tar.gz --exclude=".svn" -C ${tmpdir} rh.AmFmPmBasebandDemod-2.0.1
        rpmbuild -ta ${tmpdir}/rh.AmFmPmBasebandDemod-2.0.1.tar.gz
        rm -rf $tmpdir
    else
